...
const SolarSystem = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-black text-white">
      <div className="w-full p-6 rounded-lg bg-gradient-to-r from-indigo-900 to-purple-900 mb-4 shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-center">Cosmic Explorer</h1>
        <p className="text-center text-gray-300">Interactive 3D Star System Visualization</p>
      </div>
      
      <div className="mb-6 flex space-x-4">
        <button 
          className={`px-6 py-3 rounded-full ${currentSystem === 'solar' 
        ...
        <button 
          className={`px-6 py-3 rounded-full ${currentSystem === 'proxima' 
      ...
      <div className="mb-4 text-center text-gray-400">
        <p className="text-sm mb-1">👆 Click on a celestial body to learn about it</p>
        <p className="text-sm">🖱️ Drag to rotate | 🖲️ Scroll to zoom</p>
      </div>
      
      <div ref={containerRef} className="w-full h-96 border rounded-lg border-gray-700 shadow-2xl overflow-hidden"></div>
      
      {selectedPlanet && (
        <div className="mt-6 p-6 bg-gray-900 border border-gray-700 rounded-lg w-full max-w-2xl shadow-lg">
          <h2 className="text-2xl font-bold text-center mb-3 text-blue-300">{selectedPlanet.name}</h2>
          <p className="mb-4 text-gray-300 leading-relaxed">{selectedPlanet.description}</p>
          ...
          {showVideo && selectedPlanet.name === "Miller's Planet" ? (
            <div className="my-4">
              ...
              <div className="relative pt-2 pb-4 h-0" style={{ paddingBottom: '56.25%' }}>
                <iframe 
            ...
            <>
              <h3 className="font-semibold mb-2 text-yellow-400">Interesting Facts:</h3>
              <ul className="space-y-2">
                {selectedPlanet.facts.map((fact, index) => (
                  <li key={index} className="pl-5 relative text-gray-300">
      ...
      <div className="mt-6 p-6 bg-gray-900 border border-gray-700 rounded-lg w-full max-w-2xl shadow-lg">
        <h3 className="font-semibold mb-3 text-center text-green-400">About {currentSystem === 'solar' ? 'Our Solar System' : 'Proxima Centauri'}</h3>
      ...
      <div className="w-full text-center text-gray-600 text-xs mt-8 mb-4">
        <p>Created with Three.js & React | Cosmic Explorer v1.0</p>
      </div>
    </div>
  );
};